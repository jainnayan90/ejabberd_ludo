...
open_session(SID, User, Server, Resource, Priority, Info) ->
    %% nayan changes %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
    replace_same_resource_session(User, Server, Resource),
...
c2s_handle_info(#{lang := Lang} = State, replaced) ->
    ?WARNING_MSG("~n~n~n~n~n~n c2s_handle_info &&&&&&&&& ~p~n~n~n~n", [{}]),
...
replace_same_resource_session(User, Server, Resource) ->
    UserSessions = get_sessions(ejabberd_sm_mnesia, User, Server, Resource),
    lists:foreach(fun(Session) -> 
			  SID = Session#session.sid,
			  {User, Server, Resource} = Session#session.usr,
			  {_, PID} = SID,
			  PID ! {replaced, User, Server, Resource},
			  close_session(SID, User, Server, Resource) 
		  end, UserSessions).
