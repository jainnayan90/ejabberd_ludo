...
my_handle_cast({dice_roll, Stanza, SubEl}, State) ->
    ...
    if State#state.game_state =/= 'awaiting_dice_roll' ->
       ...
       true ->
	    ...
	    game_utils:send_result_stanza(Stanza),
	    {NewState1, Timeout1} = roll_dice(State, MovablePawns, DiceVal, Player),
	    if (length(MovablePawns) > 1) ->
		    AllStartingPawns = lists:all(fun(PawnId) ->
							 {_, Val, _} = lists:keyfind(PawnId, 1, Player#player.markers),
							 Val == 0
						 end, MovablePawns);
	       true ->
		    AllStartingPawns = false
	    end,
	    if (length(MovablePawns) == 1 orelse AllStartingPawns == true) ->
		    MovablePawn = lists:foldl(fun(PawnId, AccIn) -> 
                                                      if (AccIn == false) ->
                                                              case get_pawn_position(Player, PawnId, DiceVal) of
                                                                  error ->
                                                                      AccIn;
                                                                  {NewPlayer, Cell, SCell, CellRange} ->
                                                                      {NewPlayer, Cell, SCell, CellRange, PawnId}
                                                              end;
                                                         true ->
                                                              AccIn
                                                      end
                                              end, false, MovablePawns),
		    case MovablePawn of
                        false ->
			    {NewState, Timeout} = {NewState1, Timeout1};
                        {NewPlayer, Cell, SCell, CellRange, PawnId} ->
                            {_, _, OldCellId} = lists:keyfind(PawnId, 1, Player#player.markers),
                            {NewState, Timeout} = move_pawn(NewState1, PawnId, OldCellId, NewPlayer, Cell, SCell, CellRange, true)
                    end;
...
my_handle_cast({pawn_move, Stanza, SubEl}, State) ->
    ...
    %% From = Stanza#iq.from,
    %% To = Stanza#iq.to,
    %% Stanza1 = Stanza#iq{from=To, to=From, type=error},
    %% ?WARNING_MSG("in pawn move E2 ~p~n~n~n~n~n~n", [Stanza]),
    %% ?WARNING_MSG("in pawn move E2111111111111111111111 ~p~n~n~n~n~n~n", [Stanza1]),
    if State#state.game_state =/= 'awaiting_pawn_move' ->
       ...
       true ->
	    ...
	    OldCellId = game_utils:get_element(SubEl, <<"cell_id">>),
		...
		{NewPlayer, Cell, SCell, CellRange} ->
		    game_utils:send_result_stanza(Stanza),
		    {NewState, Timeout} = move_pawn(State, PawnId, OldCellId, NewPlayer, Cell, SCell, CellRange, false)
	...
	'awaiting_pawn_move' ->
	    ...
	    if (length(MovablePawns) > 0) ->
		    ...
		    case MovablePawn of
                        ...
                        {NewPlayer, Cell, SCell, CellRange, PawnId} ->
                            {_, _, OldCellId} = lists:keyfind(PawnId, 1, Player#player.markers),
                            {NewState, Timeout} = move_pawn(State, PawnId, OldCellId, NewPlayer, Cell, SCell, CellRange, true)
                    end;
...
send_game_info(State, Players1) ->
    ...
    Players = lists:map(fun(Player) -> 
				...
				Markers = lists:map(fun({MKey, MVal, _}) -> 
							    if MVal > 51 ->
								    Diff = MVal - 51,
...
get_pawn_position(Player, PawnId, DiceVal) ->
	...
	{PawnId, Cell, _} ->
	    if Cell == 0 andalso DiceVal == 6 ->
		    NewVal = 1,
		    NewMarkers = lists:keystore(PawnId, 1, Markers, {PawnId, NewVal, Cell}),
	       ...
	       Cell =/= 0 andalso (Cell + DiceVal) > 51 ->
		    ...
		    if NewVal < 58 ->
			    ...
			    NewMarkers = lists:keystore(PawnId, 1, Markers, {PawnId, NewVal, NewVal1}),
			    NewPlayer = Player#player{markers=NewMarkers},			    
			    CellRange =	lists:seq(Cell + 1, NewVal),
...
get_movable_pawns(Player, DiceVal) ->
    lists:foldl(fun({PawnId, _, _}, AccIn) -> 
			case get_pawn_position(Player, PawnId, DiceVal) of
			    error ->
				AccIn;
			    _ ->
...
check_for_winner(Player) ->
    HomeList = lists:all(fun({_, Val, _}) -> 
				 Val == 57
			 end, Player#player.markers).
...
check_collision(Player, Cell, CurrentIndex) ->
    ...
    ?WARNING_MSG("~n~n~n~n~n ######## IN CHECK FOR CLOOISION ~p~n~n~n~n", [{Player}]),    
    lists:foldl(fun({PawnId, Val1, ValO}, AccIn) ->
			if  Val1 < 52 andalso Val1 =/= 0->				
				%%Delta = erlang:abs((52 - erlang:abs(Player#player.index - CurrentIndex) * 13)),
				Delta = (Player#player.index - CurrentIndex) * 13,
				Val2 = Val1 + Delta,
				?WARNING_MSG("~n~n~n~n~n ######## IN CHECK FOR CLOOISION ~p~n~n~n~n", [{Val2, Delta, PawnId, Val1}]),
				if(Val2 > 52) ->
		...
		_ ->
		    ...
		    if length(CollisionMap) == 0 ->
		       ...
		       true ->
				...
				{} ->
				    {Players, false, false, false};
				{PlayerId, {PawnId, Val, OldCell}} ->
				    CPlayer = lists:keyfind(PlayerId, #player.id, Players),
				    PlayerPawns = lists:keystore(PawnId, 1, CPlayer#player.markers, {PawnId, 0, OldCell}),
...
get_pawns_player_index(Player, Cell, CurrentIndex, MarkersArray) ->
    ...
    lists:foldl(fun({PawnId, Val1, _}, AccIn) ->
			%%Delta = erlang:abs((52 - erlang:abs(Player#player.index - CurrentIndex) * 13)),
			Delta = (Player#player.index - CurrentIndex) * 13,
			Val2 = Val1 + Delta,
			if(Val2 > 52) ->
			...
			if Val1 == 0 ->
				AccIn;
			   Val1 > 51 andalso Val1 == Cell andalso CurrentIndex == Player#player.index ->
...
roll_dice(State, MovablePawns, DiceVal, Player) ->
...
move_pawn(State, PawnId, OldCellId, NewPlayer, Cell, SCell, CellRange, ForceMove) ->
    DiceVal = State#state.current_dice_val,
    Player = lists:keyfind(State#state.current_turn, #player.id, State#state.players),
    %%game_utils:send_result_stanza(Stanza),
    ...
    if (ForceMove == true) ->
	    game_utils:broadcast_stanza(State#state.players, Id, FromJID, 'set', [QueryEl1]);
       true ->
	    game_utils:broadcast_stanza_to_rest(State#state.players, Id, FromJID, 'set', [QueryEl1], Player#player.id)
    end,
    ...
    if length(CurrentCellPawns) > 1 ->
	    MIndex = #xmlel{name = <<"player_index">>, attrs = [], children = [{xmlcdata, integer_to_binary(Player#player.index)}]},
	    MCell = #xmlel{name = <<"cell_id">>, attrs = [], children = [{xmlcdata, Cell}]},
	    MQueryEl = #xmlel{name = <<"query">>, attrs = [], children = CurrentCellPawns ++ [MIndex, MCell]},
    ...
    if(SCell =/= 1) ->
	    OldCellPawns = get_pawns_on_current_index(NewPlayers1, SCell - DiceVal, NewPlayer#player.index, []),
	    ?CRITICAL_MSG("~n~n~n~n~n printing old cell pawns ~p~n~n~n~n~n~n", [OldCellPawns]),
	    if length(OldCellPawns) > 0 ->
		    OIndex = #xmlel{name = <<"player_index">>, attrs = [], children = [{xmlcdata, integer_to_binary(Player#player.index)}]},
		    OCell = #xmlel{name = <<"cell_id">>, attrs = [], children = [{xmlcdata, OldCellId}]},
		    OQueryEl = #xmlel{name = <<"query">>, attrs = [], children = OldCellPawns ++ [OIndex, OCell]},
		    OQueryEl1 = game_utils:set_namespace(OQueryEl, ?NS_PLAYER_SAME_CELL),
		    OId = game_utils:get_iq_id(FromJID, ?NS_PLAYER_SAME_CELL),
		    
		    game_utils:broadcast_stanza(State#state.players, OId, FromJID, 'set', [OQueryEl1]);
	       true ->
		    ok
	    end;
      true ->
