...
check_password(User, AuthzId, Server, Token) ->
    ...
    %%          the call stack?
    ?WARNING_MSG("in ejabberdjwt check password ~n~n~n~n~p~n~n~n~n", [{User, AuthzId, Server, Token, game_queries:get_player_info(?LSERVER, User)}]),
    case game_queries:get_player_info(?LSERVER, User) of
	error ->
	    false;
	_ ->
